...
function saveMineFrameId()
    local file = fs.open("mineframe_id.txt", "w")
    file.write(tostring(mineFrameId))
    file.close()
end

function loadMineFrameId()
    if fs.exists("mineframe_id.txt") then
        local file = fs.open("mineframe_id.txt", "r")
...
function joinRegister()
    ...
    -- Try to load saved mineFrameId
    mineFrameId = loadMineFrameId()
    
    if not mineFrameId then
        print("Enter mineFrame ID:")
        mineFrameId = tonumber(io.read())
        saveMineFrameId()  -- Save the new ID
    else
        print("Using saved mineFrame ID:", mineFrameId)
    end
